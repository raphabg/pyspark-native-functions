...
import catalyst.util.BalogoSimilarityUtils
import catalyst.expressions.codegen.{CodegenContext, ExprCode}
import catalyst.expressions.{BinaryExpression, Expression, ImplicitCastInputTypes, NullIntolerant}
import types.{AbstractDataType, DataType, DoubleType, StringType}
import org.apache.spark.unsafe.types.UTF8String


case class BalogoJaroWinklerSimilarity(left: Expression, right: Expression) extends BinaryExpression
  ...
  protected override def nullSafeEval(leftValue: Any, rightValue: Any): Any =
    BalogoSimilarityUtils.balogoJaroWinklerSimilarity(left.asInstanceOf[UTF8String], right.asInstanceOf[UTF8String])
