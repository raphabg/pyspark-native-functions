...
import org.apache.spark.sql.catalyst.expressions.BalogoFuzzyExpressions.BalogoJaroWinklerSimilarity
...
trait FunctionRegistryBase[T] {
  ...
  final def createOrReplaceTempFunction(
    ...
    registerFunction(FunctionIdentifier(name), builder, source)
  }
  ...
  def functionExists(name: FunctionIdentifier): Boolean = lookupFunction(
    name
  ).isDefined
...
object FunctionRegistryBase {
  ...
  def build[T: ClassTag](
      name: String,
      since: Option[String]
  ): (ExpressionInfo, Seq[Expression] => T) = {
...
trait SimpleFunctionRegistryBase[T]
    extends FunctionRegistryBase[T]
    with Logging {
  ...
  def internalRegisterFunction(
    ...
    functionBuilders.put(name, newFunction) match {
      case Some(previousFunction) if previousFunction != newFunction =>
        logWarning(
          s"The function $name replaced a previously registered function."
        )
      case _ =>
    }
  }
  ...
  override def lookupFunctionBuilder(
      name: FunctionIdentifier
  ): Option[FunctionBuilder] = synchronized {
...
trait FunctionRegistry extends FunctionRegistryBase[Expression] {
  ...
  override def clone(): FunctionRegistry =
...
object FunctionRegistry {
  ...
  val expressionsForTimestampNTZSupport
      : Map[String, (ExpressionInfo, FunctionBuilder)] =
  ...
  val expressions: Map[String, (ExpressionInfo, FunctionBuilder)] = Map(
    ...
    expression[BalogoJaroWinklerSimilarity]("jarowinkler"),
  ...
  private def makeExprInfoForVirtualOperator(
      name: String,
      usage: String
  ): ExpressionInfo = {
    new ExpressionInfo(
  ...
  val builtinOperators: Map[String, ExpressionInfo] = Map(
    ...
    "between" -> makeExprInfoForVirtualOperator(
      "between",
  ...
  private def expressionBuilder[T <: ExpressionBuilder: ClassTag](
    ...
    val funcBuilder = (expressions: Seq[Expression]) => {
      assert(
        ...
        "function arguments must be resolved."
      )
  ...
  private def castAlias(
    ...
    val usage =
      "_FUNC_(expr) - Casts the value `expr` to the target data type `_FUNC_`."
    val expressionInfo =
      new ExpressionInfo(
...
trait TableFunctionRegistry extends FunctionRegistryBase[LogicalPlan] {
  ...
  /** Create a copy of this registry with identical functions as this registry. */
  override def clone(): TableFunctionRegistry =
    throw new CloneNotSupportedException()
}
